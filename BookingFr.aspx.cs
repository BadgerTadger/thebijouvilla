...
public partial class BookingFr : System.Web.UI.Page
{
    ...
    protected void btnBook_Click(object sender, EventArgs e)
    {
        ...
        if (ValidFields())
        {
            ...
            {
                ClearFormFields();
                Response.Redirect("~/ThankYouFr.aspx", true);
            }
        }
        else
        {
    ...
    private bool ValidFields()
    {
        ...
        {
            retVal = false;
            lblWarning.Text += "La date de début n'est pas sélectionnée ou n'est pas une date valide.<br />";
        }
        else
        {
            ...
            {
                retVal = false;
                lblWarning.Text += "La date de début doit être sélectionnée<br />";
            }
            else if (startDate.DayOfWeek != DayOfWeek.Saturday)
            {
                retVal = false;
                lblWarning.Text += "La date de début doit être un samedi<br />";
            }
            else if (startDate < DateTime.Now)
            {
                retVal = false;
                lblWarning.Text += "Date de début ne peut pas être dans le passé<br />";
            }
            else
            {
                if (bookedDates != null)
                {
                    foreach (DateTimeConfirmed dtc in bookedDates)
                    {
                        ...
                        {
                            retVal = false;
                            lblWarning.Text += "La date de début sélectionnée n'est pas disponible<br />";
                            break;
                        }
                    }
                }
            ...
            for (DateTime date = startDate.AddDays(1); date.Date <= endDate.AddDays(-1).Date; date = date.AddDays(1))
            {
                if (bookedDates != null)
                {
                    foreach (DateTimeConfirmed dtc in bookedDates)
                    {
                        ...
                        {
                            retVal = false;
                            lblWarning.Text += "La période sélectionnée contient des dates qui ne sont pas disponibles<br />";
                            break;
                        }
                        if (!retVal)
                        {
        ...
        {
            retVal = false;
            lblWarning.Text += "Veuillez indiquer votre nom<br />";
        }
        ...
        {
            retVal = false;
            lblWarning.Text += "Veuillez compléter vos coordonnées<br />";
        }

        if (string.IsNullOrWhiteSpace(txtEmail.Text))
        {
            retVal = false;
            lblWarning.Text += "Veuillez indiquer une adresse e-mail<br />";
        }
        else if (!Utils.IsValidEmail(txtEmail.Text))
        {
            retVal = false;
            lblWarning.Text += "L'adresse e-mail fournie n'est pas valide<br />";
        }

        if (string.IsNullOrWhiteSpace(txtLandline.Text) && string.IsNullOrWhiteSpace(txtMobile.Text))
        {
            retVal = false;
            lblWarning.Text += "Veuillez fournir au moins un numéro de téléphone<br />";
        }
